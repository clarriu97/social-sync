...
export CURRENT_VERSION:=$(shell cat ${ROOTDIR}/social_sync/_meta.py  | sed -En "s/^__version__\s*=\s*[ubrf]*['\"]([^'\"]+)['\"].*$$/\1/p")
export CURRENT_USER:=$(shell id -u ${USER}):$(shell id -g ${USER})

# Docker configuration
export IMAGE_NAME:=$(or $(IMAGE_NAME), social_sync)
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) social_sync || true
...
lint: ## static code analysis with pylint
	pylint --rcfile social_sync/.pylintrc social_sync
...
code-locs: ## display metrics (LOCs, number of comments, etc.)
	radon raw --summary social_sync

code-complexity: ## check cyclomatic complexity using radon
	xenon --max-absolute B --max-modules A --max-average A social_sync
...
docker-run: ## run the specified command on docker (defaults to /bin/bash)
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) social_sync $(or $(COMMAND),  $(MAKE) $(TARGET))

docker-shell: ## drop the user into a shell inside a docker container
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) social_sync
...
docker-package: ## package an image as a tar archive
	...
	docker save -o ${ROOTDIR}/dist/social_sync-$(IMAGE_VERSION).tar $(IMAGE_NAME):$(IMAGE_VERSION)
...
clean-build: ## remove build artifacts
	...
	find social_sync -name '*.c' -exec rm -f {} +
